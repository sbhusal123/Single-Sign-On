...
class Server:
    ...
    def get_urls(self):
        return [
            path('request-token/', provider_for_django(self.request_token_provider(server=self)),
                    name='simple-sso-request-token'),
            path('authorize/', self.authorize_view.as_view(server=self), name='simple-sso-authorize'),
            path('verify/', provider_for_django(
